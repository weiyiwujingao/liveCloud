...
<link type="text/css" rel="stylesheet" href="http://hs.cnfol.com/f=Cm/Css/Base.css,ua/css/cloud/window.css,ua/css/cloud/Inner.css,ua/css/cloud/Calendar.css,ug/Css/Back2/Ins/Insback.css,ub/Css/NetGold/File.css" />
...
<?php date_default_timezone_set('Asia/Chongqing');?>
    ...
    <p class="SearBar">
	 ...
	 <?php if($add){?><a href="javascript:void(0);" class="addBtn" onClick="Dialog('TMaddUserTmk');">添加</a><?php }?>
    ...
    <?php 
      ...
      <tr class="<?php echo $i%2==0?'':'evenTr'?>" id="<?php echo $item->id ;?>">
        ...
        <td class="spTd"><?php if($edit){?><a href="javascript:void(0);" class="btnStyleA L" onClick="EditUserMsg('<?php echo $item->id;?>');">修改</a><?php }?><?php if($del){?><a onClick="DelRecord(<?php echo $item->id?>);" class="btnStyleA L" href="javascript:void(0);">删除</a><?php }?></td>
      </tr>
      <?php $i++;}}?>
        ...
        <dd class="mgTpCont" style="display:block;">
		  <form id="Sm1" class="vFmV" enctype="multipart/form-data" method='post' name='Sm1' action="<?php echo yii::app()->createUrl('user/Img_Upload');?>">
			...
			<label class="labelTd03 La" for="nick_name"><i class="W80">中金昵称：</i><input type="text" value="" id="nick_name" /></label>
              <label class="labelTd03 La" for="nick_name"><i class="W80">嘉宾专栏：</i><input type="text" value="" id="zhuanlan" name='zhuanlan'></label>
			<label class="hAuto labelTd03 Pst"><i class="W130">嘉宾头像：&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</i><!-- <input type="text" class="W190" id="AddNm4" /> -->

			<img src="" class="rePic" style="float:left;width:80px;height:80px" id="imgpath"  /><a href="javascript:Dialog.Mask(C.G('TMImgsUploadTmk'));" class="sentPicBtn" onClick="Dialog('TMImgsUploadTmk');" >上传图片</a>
			</label>
  ...
  <dl class="mgTpMain w530 L">
        ...
        <input type="hidden" class="norTxtBox" id="aid"/>
		  <form id="Sm2" name="Sm2" class="vFmV">
			...
			<img src="" class="rePic" style="float:left;width:80px;height:80px" id="imgpath1"  /><a href="javascript:Dialog.Mask(C.G('TMImgsUploadTmk'));" class="sentPicBtn" onClick="Dialog('TMImgsUploadTmk');" >上传图片</a>
			</label>
...
<script charset="utf-8" src="http://hs.cnfol.com/f=Cm/Js/Base.js,Cm/Js/Dialog.js,Cm/Js/Tabs.js,Cm/Js/Checkbox.js,ua/js/Clouds/Tables.js,ua/js/Clouds/Calendar.js,Cm/Js/Menus.js,ub/Js/NetGold/File.js,ud/Js/Guijs/uploadPreview.min.js" type="text/javascript"></script>
...
function Search(){
	var url = "";
	if(C.G('username')){
		var url="&username="+C.G('username').value;
	}
	if(url){
