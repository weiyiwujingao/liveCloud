...
<dl id="C1a" class="NTabs PstR">
        ...
        <form class="FmSrch ComWrp" id="Search" name="Search">
            ...
            <label style="vertical-align:middle">状态：
                <select id="audit" name='audit'>
                    ...
                    <option <?php if(isset($_GET['audit'])&& $_GET['audit']==0){echo 'selected="selected"';} ?> value='0'>未审核</option>
                    <option <?php if(isset($_GET['audit'])&& $_GET['audit']==1){echo 'selected="selected"';} ?> value ='1'>已审核</option>
					<option <?php if(isset($_GET['audit'])&& $_GET['audit']==3){echo 'selected="selected"';} ?> value ='3'>审核未通过</option>
                </select>
            </label>
	...
	<?php 
		...
		{
	?>
	<a href="javascript:void(0);" class="BtnSrch TitBtn1 R" onClick="DelRecord(0,1);">批量审核</a>
	<?php 
		}
	?>
	查询结果：</p>
		...
		<?php 
			if(!empty($infoList))
			{
				...
				{	
					...
					echo '<td class="Nrp">'.$val['userip'].'</td>';
					echo '<td class="Nrp" title="'.$ask.'">'.$ask.'</td>';
					echo '<td class="Nrp" title="'.$val['replyer'].'">'.$val['replyer'].'</td>';
					echo '<td class="Nrp" title="'.$val['question'].'">'.$val['question'].'</td>';
					echo '<td class="Nrp">'.date('Y-m-d H:i:s',$val['atime']).'</td>';
					echo '  <td class="Nrp">'.$status[$val['audit']].'</td>';
					echo '  <td class="spTd">';
					if($mod&&$val['audit']==0)
					{
						echo '<a onClick="DelRecord('.$val['id'].',1);" class="btnStyleA L" href="javascript:void(0);">审核</a>';
					}
					if($del)
					{
...
<div id="TMPasTmk" class="MsContent">
	<div class="mgTpMain w250 L">
    	<div class="mgTpTitle">审核</div>
        <div class="mgTpCont">
        	<p>
            	<label class="PasLab"><input  checked="checked" type="radio" value="1" name="PasRdo">通过</label>
            	<label class="PasLab"><input type="radio" value="3" name="PasRdo">不通过</label>
            </p>
        	<span><a href="javascript:void(0);" class="sumitBtn" id="SurePas">确定</a><a href="javascript:void(0);"  class="cancelBtn" onClick="Dialog.Close();">取消</a></span>	
        </div>
    </div>
    <a href="javascript:void(0);" class="closeBtn L" onClick="Dialog.Close();"></a>
</div>
...
function MakeSureDel(id,type){
	...
	if(type==1)
	{	
		Data+="&pasrdo="+$(':radio[name="PasRdo"]:checked').val();
	}
