...
class Images{
	var $inputName;                 //控件名
	var $allowType = array('image/gif','image/jpg','image/jpeg','image/pjpeg');	                           //上传类型
	...
	function move_uploaded(){        //移动上传文件
		...
		/*FTP连接信息*/
		$config_CT = array(
					'hostname' => Yii::app()->params['system']['ftpCT']['ftpHost'],
					'username' => Yii::app()->params['system']['ftpCT']['ftpUserName'],
					'password' => Yii::app()->params['system']['ftpCT']['ftpPwd'],
					'port' => Yii::app()->params['system']['ftpCT']['ftpPort']
					);
		$config_CNC = array(
					'hostname' => Yii::app()->params['system']['ftpCNC']['ftpHost'],
					'username' => Yii::app()->params['system']['ftpCNC']['ftpUserName'],
					'password' => Yii::app()->params['system']['ftpCNC']['ftpPwd'],
					'port' => Yii::app()->params['system']['ftpCNC']['ftpPort']
					);
		$last = $this->saveDir.DIRECTORY_SEPARATOR.$file_newname ;//文件本地目录
		//$remote = Yii::app()->params['system']['ftpCNC']['ftpImgPath'].$file_newname ;
		$remote = $file_newname;
		
		/*电信*/			
		$ftp_CT = new Ftp();
		$ftp_CT->connect($config_CT);
		$result_CT = $ftp_CT->upload($last,$remote);
		//return $result_CT;
		
		/*网通*/
		$ftp_CNC = new Ftp();
		$ftp_CNC->connect($config_CNC);
		$result_CNC = $ftp_CNC->upload($last,$remote);
		
		if( $result_CNC){
			@unlink($last);
			$path = Yii::app()->params['system']['ftpCNC']['ftpImgPath'].$remote;//文件的成功保存路径
			return $path;
		}else{
       ...
       $info  = $this->getImageInfo($image);
         if($info !== false) {
            ...
            if ($suofang==1) {
                ...
                $thumbImg01 = imagecreatetruecolor(120,160);
                imagecopyresampled($thumbImg01,$thumbImg,0,0,$_POST['x'],$_POST['y'],190,195,$_POST['w'],$_POST['h']);

				$thumbImg02 = imagecreatetruecolor(120,160);
                imagecopyresampled($thumbImg02,$thumbImg,0,0,$_POST['x'],$_POST['y'],50,50,$_POST['w'],$_POST['h']);
...
//	./uploads/2011/6/23/201106231015231234985802.jpg	----->裁剪的小头像	50*50
