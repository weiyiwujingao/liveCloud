...
class Images1{
	...
	var $saveDir = "/home/httpd/cloud_computing_center/roadShow.cloud.cnfol.com/images/";     //保存目录
	...
	function move_uploaded(){        //移动上传文件
		...
		$this->saveDirtmp = $this->saveDir.date('Ym');
		if(!file_exists($this->saveDirtmp)){       //判断保存目录是否存在
			mkdir($this->saveDirtmp,0777,true);    //建立保存目录
		}
		//移动文件
		$result = move_uploaded_file($_FILES[$this->inputName]['tmp_name'],$this->saveDirtmp."/".$file_newname);

		if($result){
			$path = $this->savePath = $this->saveDirtmp."/".$file_newname;		//文件的成功保存路径
			return $path;
		}else{
    ...
    function thumb($image,$is_save=true,$suofang=0,$type='',$maxWidth=1000,$maxHeight=1000,$interlace=true){
        // 获取原图信息
       $info  = $this->getImageInfo($image);
         if($info !== false) {
            ...
            if ($suofang==1) {
				...
				$thumbImg02 = imagecreatetruecolor(120,120);
                imagecopyresampled($thumbImg02,$thumbImg,0,0,$_POST['x'],$_POST['y'],120,120,$_POST['w'],$_POST['h']);
...
//	./uploads/2011/6/23/201106231015231234985802.jpg	----->裁剪的小头像	120*120
